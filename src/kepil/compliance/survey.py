...
def journal_intact(_: Check) -> Answer:
    ...
    if seen or signed:
        return Answer(NEEDS_HUMAN,
                      "цепочка сходится, и корни назывались человеку. Но перечень "
                      "карточек берётся из самого журнала: обрезав историю по "
                      "последнее подтверждение, можно получить хвост без свидетелей. "
                      "Проверить удаление способен только перечень, составленный "
                      "снаружи — в канале, куда уходили карточки",
                      evidence + [
                          "сверка снаружи внутрь: verify_against_sent(журнал, перечень)",
                          "отдельно: сверка корня при нажатии защищает работающую "
                          "систему, но сравнивает с местным состоянием очереди — "
                          "переписавший журнал перепишет и её, доказательством "
                          "задним числом это не является",
                      ])
