...
> **Разбираетесь с Законом РК № 230-VIII об искусственном интеллекте?**
> Отдельно лежит практическое руководство: что считается системой ИИ, как
> классифицировать риск и автономность, что запрещено всегда, нужен ли вам
> аудит и с чего начать —
> [«Закон об искусственном интеллекте в Казахстане: что делать бизнесу»](docs/kz-230-practical.md).
...
## Свидетелем журнала работает подтверждение

Цепочка хешей доказывает, что уцелевшие записи согласуются друг с другом. О том,
что из журнала убрали, она не говорит ничего: обрежьте на третьей записи,
пересчитайте остальные с верными `prev_hash` — и проверка скажет «целостность
подтверждена» над укороченной историей, согласованной сама с собой.

Средство от этого уже было в устройстве системы, просто не использовалось.
Карточка подтверждения уходит из процесса-писателя к человеку — в панель или в
Telegram, — и этот обмен единственный, что рождается за пределами писателя.
Поэтому на карточке теперь стоит текущий корень цепочки, вернувшееся решение
цитирует корень, который видело, и он попадает в журнал.

Переписанному началу приходится теперь противоречить сообщению, лежащему там,
куда писатель не дотянется. Проверка перестаёт быть проверкой на
самосогласованность и становится показанием второй стороны о том, как журнал
выглядел в такой-то момент.

```
цепочка сама с собой:    целостность подтверждена
названный корень:        подтверждение в записи 0 ссылается на корень
                         sha256:a3d124b9…, которого в цепочке нет: история
                         была обрезана или переписана (таких подтверждений: 3)
```

Записи, сделанные после последнего названного корня, остаются без защиты. Там
гарантия и заканчивается, и система об этом говорит.

Это подсказал читатель, [ANP2 Network](https://dev.to/olegvdv/i-tried-to-forge-my-own-ai-agents-audit-log-3chh),
описавший в одном комментарии и атаку, и средство.
